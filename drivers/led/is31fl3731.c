...
static int is31fl3731_write_reg(const struct i2c_dt_spec *i2c, uint8_t reg,
				 uint8_t val)
{
	uint8_t buffer[2] = {reg, val};

	return is31fl3731_write_buffer(i2c, buffer, sizeof(buffer));
}

static int is31fl3731_write_reg8(const struct i2c_dt_spec *i2c, uint8_t bank, uint8_t reg, uint8_t data) {
	int status;
	LOG_ERR("IS31FL3731 REG:%d to %d", reg, data);
...
static int is31fl3731_led_on(const struct device *dev, uint32_t led) {
	return is31fl3731_led_set_brightness(dev, led, 100);
}

static int is31fl3731_led_off(const struct device *dev, uint32_t led) {
	return is31fl3731_led_set_brightness(dev, led, 0);
}

static void is31fl3731_clear(const struct i2c_dt_spec *i2c) {
  uint8_t erasebuf[25];
